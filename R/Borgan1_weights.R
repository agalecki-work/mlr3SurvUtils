# ' Calculate Borgan I Weights for Case-Cohort Study
# '
# ' This function calculates Borgan I weights for either a survival analysis task
...
# '
# ' @details
# ' This function adjusts weights according to the Borgan I method,
...
# ' weights_surv <- Borgan1_weights(df_surv, "subcoh", "event", 0.15, "surv")
# ' print(weights_surv)
# '
# ' # Classification task example:
# ' df_classif <- data.frame(subcoh = c(1, 0, 1, 0, 1), 
# '                  status = factor(c('censored', 'main', 'competing', 'main', 'censored')))
# ' weights_classif <- Borgan1_weights(df_classif, "subcoh", "status", 0.15, "classif")
# ' print(weights_classif)
# '
# ' @export
Borgan1_weights <- function(data, subcoh, event_or_status, sampling_fraction=0.15, task_type="surv") {
